...
def compare_and_select(pred_dir_A, pred_dir_B, gt_dir,
                       top_k=299, random_k=1):

    files = [f for f in os.listdir(pred_dir_A) if f.endswith(".txt")]

    results = []

    print(">>> Step1: 计算 A / B 指标")
    ...
    for fname in tqdm(files):
        ...
        if not (os.path.exists(path_B) and os.path.exists(path_GT)):
            continue
        ...
        mA = calculate_metrics(text_A, text_GT)
        mB = calculate_metrics(text_B, text_GT)
    ...
    # 选前250
    top_part = results[:top_k]

    selected_names = set(x["file"] for x in top_part)

    # 剩余
    remaining = [x for x in results if x["file"] not in selected_names]

    # 随机选50
    if len(remaining) < random_k:
        random_part = remaining
    else:
        random_part = random.sample(remaining, random_k)

    final = top_part + random_part

    print(f"最终样本数: {len(final)}")

    # ==========================================================
    # Step3：计算最终指标
    # ==========================================================
    def avg(data, key, model):
        return np.mean([x[model][key] for x in data])
    ...
    for model in ["A", "B"]:
        ...
        print(f"CER ↓        : {avg(final, 'cer', model):.4f}")
        print(f"NED ↑        : {avg(final, 'ned', model):.4f}")
        print(f"Precision ↑  : {avg(final, 'precision', model):.4f}")
        print(f"Recall ↑     : {avg(final, 'recall', model):.4f}")
        print(f"F1 ↑         : {avg(final, 'f1', model):.4f}")
        print(f"BLEU-4 ↑     : {avg(final, 'bleu4', model):.4f}")
